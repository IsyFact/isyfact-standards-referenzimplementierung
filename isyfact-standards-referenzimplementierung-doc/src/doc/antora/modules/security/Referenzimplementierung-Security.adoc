...
Entscheidend für die in den von Keycloak gelieferten Tokens (und für die in den Tokens gelieferten Berechtigungen)
ist weiterhin die in Keycloak persistierte Konfiguration.
Zum Verständnis der Testfälle empfehlen wir, sich ggf. über den Keycloak-Client diese Konfiguration genauer anzusehen.

==== Security-Consumer-Tests

|====
|*Testklasse*|*Testfall*|*Testtechnologie*
|AntragControllerApiTest|Antrag aktualiseren|API-Test, inklusive Aufruf eines externen Service.
|====
...
|====
*Anmerkung:*
Mit den AuthenticationManagerTests wird Funktionalität der Methoden des AuthenticationManager-Interfaces überprüft.
...
Ziel der PublicResourceTests und SecuredRourceTests war es, die Funktionsweise der '@Secured' Annotation zu prüfen.

Ziel Api-Tests ist es, die REST-Api der beiden Anwendungen technisch und fachlich zu prüfen
und dabei auch indirket (über eine lange Aufrufkette) sowohl Token-Ermittling und -Weitergabe auf Service-Consumer-Seite,
wie auch Token-Validierung auf Service-Producer-Seite abzusichern.